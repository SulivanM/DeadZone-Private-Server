package thelaststand.app.game.gui.chat.components
{
   ...
   import thelaststand.app.game.gui.dialogues.AllianceInvitationDialogue;
   import thelaststand.app.network.Network;
   import thelaststand.app.network.chat.ChatMessageData;
   import thelaststand.common.lang.Language;
   
   public class ChatMessageDisplay_Alliance extends Sprite implements IChatMessageDisplay
   {
      private static const DEFAULT_ICON:BitmapData = new BmpIconAllianceSmall();
      ...
      private var _acceptRejectStr:String = "<font color=\'#6eb91c\'><b><a href=\'event:accept\'>" + this._lang.getString("alliance.chat_linkAccept") + "</a></b></font>  <font color=\'#dd785d\'><b><a href=\'event:reject\'>" + this._lang.getString("alliance.chat_linkReject") + "</a></b></font>";
      
      public function ChatMessageDisplay_Alliance()
      {
         super();
         this._bg = new Shape();
         addChild(this._bg);
         this._icon = new Bitmap(DEFAULT_ICON);
         this._icon.x = 2;
      ...
      {
         return new AllianceMessageDisplayData(param1);
      }
      ...
      public function populate(param1:IChatMessageDisplayData) : void
      {
         if(this._messageData)
         {
            ...
            AllianceMessageDisplayData(this._messageData).onExpiration.remove(this.refreshLayout);
         }
         this._messageData = param1;
         if(this._messageData)
         {
            this._messageData.display = this;
            AllianceMessageDisplayData(this._messageData).onExpiration.add(this.refreshLayout);
            this._icon.bitmapData = AllianceMessageDisplayData(this._messageData).iconBitmapData;
         }
         this.refreshLayout();
      }
      ...
      private function redrawBG() : void
      {
         ...
         _loc1_.beginGradientFill(GradientType.LINEAR,[6447714,4079166],[1,1],[0,255],_loc3_);
      ...
      private function refreshLayout() : void
      {
         ...
         var _loc1_:AllianceMessageDisplayData = AllianceMessageDisplayData(this._messageData);
         if(_loc1_.expired)
         {
            switch(_loc1_.status)
            {
               case AllianceMessageDisplayData.STATUS_ACCEPTED:
                  this._textLinks.htmlText = this._lang.getString("alliance.chat_linkAccepted");
                  break;
               case AllianceMessageDisplayData.STATUS_REJECTED:
                  this._textLinks.htmlText = this._lang.getString("alliance.chat_linkRejected");
                  break;
               case AllianceMessageDisplayData.STATUS_EXPIRED:
               default:
                  this._textLinks.htmlText = this._lang.getString("alliance.chat_linkExpired");
            }
         ...
         var _loc2_:Number = this._width - (this._label.x + this._linkSpaceRequired) - 5;
         if(this._label.width > _loc2_)
         {
            this._label.scaleX = this._label.scaleY = _loc2_ / this._label.width;
         }
      ...
      private function onTextLink(param1:TextEvent) : void
      {
         ...
         var _loc5_:AllianceMessageDisplayData = AllianceMessageDisplayData(this._messageData);
         switch(_loc3_)
         {
            ...
            case ChatLinkEvent.LT_ALLIANCE_SHOW:
               ...
               break;
            case "accept":
               _loc6_ = Boolean(_loc5_.messageData.linkData) && _loc5_.messageData.linkData.length > 1 ? _loc5_.messageData.linkData[1] : "";
               Network.getInstance().chatSystem.sendAllianceInviteResponse(_loc5_.messageData.posterNickName,1,_loc6_);
               new AllianceInvitationDialogue(_loc5_.messageData.posterAllianceId,_loc5_.messageData.posterNickName,_loc6_).open();
               _loc5_.expire(AllianceMessageDisplayData.STATUS_ACCEPTED);
               break;
            case "reject":
               _loc6_ = Boolean(_loc5_.messageData.linkData) && _loc5_.messageData.linkData.length > 0 ? _loc5_.messageData.linkData[1] : "";
               Network.getInstance().chatSystem.sendAllianceInviteResponse(_loc5_.messageData.posterNickName,0,_loc6_);
               _loc5_.expire(AllianceMessageDisplayData.STATUS_REJECTED);
               break;
            default:
               return;
         }
      ...
      {
         return UIChatMessageList.MESSAGE_TYPE_ALLIANCE;
      }
   }
}
...
class AllianceMessageDisplayData implements IChatMessageDisplayData
{
   ...
   public var onExpiration:Signal;
   ...
   public function AllianceMessageDisplayData(param1:ChatMessageData)
   {
      var _loc3_:AllianceBannerDisplay = null;
      this.onExpiration = new Signal();
      super();
      this._cmd = param1;
      ...
      this._msg = Language.getInstance().getString("alliance.chat_allianceInviteMsg");
      this._msg = this._msg.replace("%user",_loc2_);
      this._bitmapData = new BmpIconAllianceSmall();
      if(param1.linkData.length > 0 && param1.linkData[0] != "")
      {
         _loc3_ = AllianceBannerDisplay.getInstance();
         if(_loc3_.ready == false)
         {
            _loc3_.onReady.addOnce(this.updateBitmapData);
         }
         else
         {
   ...
   private function updateBitmapData() : void
   {
      ...
      _loc1_.hexString = this._cmd.linkData[0];
      this._bitmapData.fillRect(this._bitmapData.rect,0);
      var _loc2_:BitmapData = _loc1_.generateChatThumbnail();
      this._bitmapData.copyPixels(_loc2_,_loc2_.rect,new Point());
      _loc2_.dispose();
   }
