package thelaststand.app.game.gui.raid
{
   ...
   public class RaidLaunchDialogue extends BaseDialogue
   {
      private var _raidName:String;
      ...
      public function RaidLaunchDialogue(param1:String)
      {
         var bgPadding:int;
         var raid:XML;
         ...
         var raidName:String = param1;
         this.mc_container = new Sprite();
         super("raid-launch",this.mc_container,true);
         this._raidName = raidName;
         _autoSize = false;
         addTitle(Language.getInstance().getString("raid." + this._raidName + ".launch_title"),BaseDialogue.TITLE_COLOR_GREY,-1,new BmpBountySkull());
         btnLaunch = PushButton(addButton(Language.getInstance().getString("raid.launch_raid"),true,{"width":160}));
         ...
         this.ui_image.uri = "images/raids/" + this._raidName + "_launch.jpg";
         ...
         this.txt_desc.htmlText = Language.getInstance().getString("raid." + this._raidName + ".launch_desc");
         ...
         raid = ResourceManager.getInstance().getResource("xml/raids.xml").content.raid.(@id == raidName)[0];
         levelMin = int(raid.level_min);
         levelMax = int(raid.level_max);
      ...
      {
         var _loc2_:RaidDialogue = new RaidDialogue(this._raidName);
         _loc2_.open();
      }
   }
}
