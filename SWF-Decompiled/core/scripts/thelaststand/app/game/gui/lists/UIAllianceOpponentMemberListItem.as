package thelaststand.app.game.gui.lists
{
   ...
   public class UIAllianceOpponentMemberListItem extends UIPagedListItem
   {
      ...
      private var txt_win:BodyTextField;
      
      private var txt_lose:BodyTextField;
      
      private var txt_noScore:BodyTextField;
      ...
      public function UIAllianceOpponentMemberListItem()
      {
         var _loc3_:UIListSeparator = null;
         this.COL_WIDTHS = new <int>[95,134,80,170];
         super();
         ...
         this.ui_portrait = new UIImage(52,52,0,1,true);
         this.ui_portrait.context = new LoaderContext(true);
         this.ui_portrait.graphics.beginFill(0);
         this.ui_portrait.graphics.drawRect(-1,-1,54,54);
         ...
         this.txt_win = new BodyTextField({
            "text":" ",
            "color":6145600,
            "size":16,
            "bold":true,
         ...
         this.txt_lose = new BodyTextField({
            "text":" ",
            "color":11542046,
            "size":12,
            "bold":true,
         ...
         this.txt_noScore = new BodyTextField({
            "text":"-",
            "color":5592405,
            "size":16,
            "bold":true,
         ...
         this.txt_noScore.y = int((this.mc_background.height - this.txt_noScore.height) * 0.5);
         ...
         this.btn_attack = new PushButton(this._lang.getString("alliance.oppenentList_attack"),null,-1,{"bold":true});
         this.btn_attack.clicked.add(this.onClickButton);
         this.btn_attack.enabled = false;
         this.btn_attack.backgroundColor = 7545099;
         this.btn_attack.width = this.btn_view.width;
         this.btn_attack.y = int((_height - this.btn_attack.height) * 0.5);
      ...
      override public function dispose() : void
      {
         ...
         this.txt_lose.dispose();
         this.txt_lose = null;
         this.txt_win.dispose();
         this.txt_win = null;
         this.txt_noScore.dispose();
         this.txt_noScore = null;
      ...
      private function update() : void
      {
         ...
         if(this._member == null)
         {
            ...
            if(this.txt_win.parent != null)
            {
               this.txt_win.parent.removeChild(this.txt_win);
            }
            if(this.txt_lose.parent != null)
            {
               this.txt_lose.parent.removeChild(this.txt_lose);
            }
            if(this.txt_noScore.parent != null)
            {
               this.txt_noScore.parent.removeChild(this.txt_noScore);
            }
            if(this.ui_busy.parent != null)
            {
               this.ui_busy.parent.removeChild(this.ui_busy);
            }
            if(this.btn_attack.parent != null)
            {
               this.btn_attack.parent.removeChild(this.btn_attack);
            }
         ...
         if(Network.getInstance().serverTime >= AllianceSystem.getInstance().round.activeTime.time && this._remotePlayerData != null && this._remotePlayerData.canAttack() && _loc4_ == true)
         {
            this.txt_win.text = this._lang.getString("alliance.oppenentList_win",this._member.raidWinPts.toString()).toUpperCase();
            this.txt_win.x = int(_loc2_ + (this.COL_WIDTHS[2] - this.txt_win.width) * 0.5);
            addChild(this.txt_win);
            this.txt_lose.text = this._lang.getString("alliance.oppenentList_loss",this._member.raidLosePts.toString()).toUpperCase();
            this.txt_lose.x = int(_loc2_ + (this.COL_WIDTHS[2] - this.txt_lose.width) * 0.5);
            if(this._member.raidLosePts != 0)
            {
               addChild(this.txt_lose);
               this.txt_win.y = 12;
               this.txt_lose.y = 28;
            }
            else
            {
               if(this.txt_lose.parent)
               {
                  removeChild(this.txt_lose);
               }
               this.txt_win.y = 18;
            }
            if(this.txt_noScore.parent)
            {
               this.txt_noScore.parent.removeChild(this.txt_noScore);
            }
            this._displayedWarPoints = this._member.raidWinPts;
         }
         else
         {
            this.txt_noScore.x = int(_loc2_ + (this.COL_WIDTHS[2] - this.txt_noScore.width) * 0.5);
            addChild(this.txt_noScore);
            if(this.txt_win.parent)
            {
               this.txt_win.parent.removeChild(this.txt_win);
            }
            if(this.txt_lose.parent)
            ...
            this._displayedWarPoints = 0;
         }
         _loc2_ += this.COL_WIDTHS[2];
         this.btn_view.x = _loc2_ + 20;
         this.btn_attack.x = this.btn_view.x + this.btn_view.width + 18;
         ...
         if(this._remotePlayerData == null)
         {
            ...
            this.btn_view.enabled = false;
            this.btn_attack.label = this._lang.getString("alliance.oppenentList_loading");
            this.btn_attack.enabled = false;
            ...
            this._tooltips.add(this.btn_attack,this._lang.getString("alliance.tooltip_loadingData"),new Point(NaN,0),TooltipDirection.DIRECTION_DOWN);
         ...
         else
         {
            ...
            this.btn_attack.enabled = this._remotePlayerData.canAttack() && _loc4_;
            this.btn_attack.label = this._lang.getString("alliance.oppenentList_attack");
            _loc5_ = _loc4_ ? RemotePlayerData.getAttackToolTip(this._remotePlayerData) : Language.getInstance().getString("alliance.tooltip_playerNotActive");
            this._tooltips.add(this.btn_attack,_loc5_,new Point(NaN,0),TooltipDirection.DIRECTION_DOWN);
      ...
      public function getSortValue(param1:String) : *
      {
         switch(param1)
         {
            case "attack":
               if(this._remotePlayerData != null)
               {
                  return this._remotePlayerData.canAttack() ? this._displayedWarPoints : -this._displayedWarPoints;
               }
               return this._displayedWarPoints;
               break;
            case "online":
