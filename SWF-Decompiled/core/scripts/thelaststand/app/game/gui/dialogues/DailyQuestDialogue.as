package thelaststand.app.game.gui.dialogues
{
   ...
   public class DailyQuestDialogue extends BaseDialogue
   {
      ...
      private function generateSurvivorRequestText(param1:Survivor) : String
      {
         ...
         while(_loc2_ == null)
         {
            _loc2_ = _loc7_.length() > 0 ? _loc7_[int(Math.random() * _loc7_.length())] : "";
            if(_loc2_.indexOf("%srv%") > -1)
            {
               _loc12_ = Network.getInstance().playerData.compound.survivors;
               if(_loc12_.length != 2)
               {
                  while(_loc13_ == null)
                  {
                     _loc13_ = _loc12_.getSurvivor(int(Math.random() * _loc12_.length));
                     if(_loc13_ == param1 || _loc13_ == Network.getInstance().playerData.getPlayerSurvivor())
                     {
                        _loc13_ = null;
                     }
                  }
                  _loc2_ = _loc2_.replace(/%srv%/ig,_loc13_.firstName);
                  break;
               }
               _loc2_ = null;
            }
         }
         _loc3_ = _loc8_ != null && _loc8_.length() > 0 ? _loc8_[int(Math.random() * _loc8_.length())] : "";
         _loc4_ = _loc9_ != null && _loc9_.length() > 0 ? _loc9_[int(Math.random() * _loc9_.length())] : "";
         if(_loc11_ != null)
         {
            _loc14_ = Language.getInstance().getString("items." + _loc11_).toLowerCase();
            _loc2_ = _loc2_.replace(/%res%/ig,_loc14_);
            _loc3_ = _loc3_.replace(/%res%/ig,_loc14_);
            _loc4_ = _loc4_.replace(/%res%/ig,_loc14_);
         }
