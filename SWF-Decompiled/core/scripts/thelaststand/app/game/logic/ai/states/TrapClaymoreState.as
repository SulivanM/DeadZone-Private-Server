package thelaststand.app.game.logic.ai.states
{
   ...
   import thelaststand.app.game.entities.effects.DirectionalExplosion;
   import thelaststand.app.game.logic.ai.AIActorAgent;
   
   public class TrapClaymoreState implements IAITrapState
   {
      ...
      public function TrapClaymoreState(param1:Building)
      {
         super();
         this._agent = param1;
         ...
         this._arcCosine = Math.cos(45 * Math.PI / 180);
      }
      ...
      {
         this._forward = this._agent.buildingEntity.transform.forward;
         ...
         for(; _loc4_ < _loc5_; _loc4_++)
         {
            ...
            if(_loc6_.health > 0)
            {
               ...
               if(_loc9_ < this._detectionRadius * this._detectionRadius)
               {
                  ...
                  _loc7_ *= _loc9_;
                  _loc8_ *= _loc9_;
                  _loc10_ = _loc7_ * -this._forward.x + _loc8_ * -this._forward.y;
                  if(!(_loc10_ <= 0 && this._arcCosine * 0.5 > _loc10_))
                  {
                     this.trigger();
                     break;
                  }
      ...
      {
         var _loc1_:DirectionalExplosion = new DirectionalExplosion(this._agent,this._worldCenter.x,this._worldCenter.y,0,new Vector3D(-this._forward.x,-this._forward.y,-this._forward.z),this._arcCosine,this._damage,this._range,this._agent.blackboard.enemies,this._agent.getSound("fire"));
